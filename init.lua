...
--MY CUSTOM KEYS
-- Przesuń zaznaczone linie w dół
vim.keymap.set("v", "J", ":move '>+1<CR>gv=gv")
-- Przesuń zaznaczone linie w górę
...
require("lazy").setup({
	...
	-- KAMIL
	{
		...
		config = function()
			require("nuget").setup({
				-- zostaw puste: ma domyślne keymapy i działa od razu
				-- keys = {} -- (opcjonalnie) jeśli chcesz wyłączyć domyślne skróty
			})
			-- (opcjonalnie) własne skróty, jeśli wolisz:
			-- vim.keymap.set("n", "<leader>na", "<cmd>NuGetInstall<CR>", { desc = "NuGet: Add package" })
			-- vim.keymap.set("n", "<leader>nr", "<cmd>NuGetRemove<CR>",  { desc = "NuGet: Remove package" })
		end,
	},
...
-- === C# snippets: class/interface with auto-namespace (LuaSnip) ===
do
	...
	local function compute_namespace(file_path)
		...
		if not proj_dir or not proj_name then
			return vim.fn.fnamemodify(vim.fn.getcwd(), ":t") -- fallback
		end

		local rel = file_dir:sub(#proj_dir + 2) -- +2 = separator
		if rel == "" then
			return proj_name
		end
		rel = rel:gsub("[/\\]", ".")
	...
	local function add_cs_snippets()
	...
	-- gdy Lazy już wstał i pluginy są dostępne
